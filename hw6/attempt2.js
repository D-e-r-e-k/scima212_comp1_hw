...
function getSteer() {
    let x = map(mouse_vec.x, -width/6, width/6, -PI/180, PI/180, true); 
    let y = -map(mouse_vec.y, -width/6, width/6, -PI/180, PI/180, true);
    //console.log(y);
...
function showVol() {
    ...
    let x = map(delta.x, -2, 2, -width/6, width/6);
    let y = map(delta.y, -2, 2, -width/6, width/6);
